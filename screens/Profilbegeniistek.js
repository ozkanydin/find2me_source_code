...
if(kadi == this.props.kadi){
        ...
        firebase.database().ref("uye_detay/"+this.props.kadi+"/begeniistek").orderByChild("kadiii").on('value', (snap) => {
      let user = [];
      snap.forEach((konum) => {
...
top:      40,}}>
        ...
        <FlatList 
          ...
          renderItem={({item}) => 
       <ScrollView>
      <TouchableOpacity onPress={ () => Actions.detaybegeniistek({kadi : item.kadi,kadimm : this.state.kadii}) }>
