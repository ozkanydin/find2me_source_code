...
if(kadi == this.props.kadi){
        ...
        firebase.database().ref("uye_detay/"+this.props.kadi+"/begendiklerin").orderByChild("kadiii").on('value', (snap) => {
      let user = [];
      snap.forEach((konum) => {
...
top:      40,}}>
      ...
      <TouchableOpacity onPress={ () => Actions.detaybegenilen({kadi : item.kadi,kadimm : this.state.kadii})  }>
