...
import {Platform,BackHandler,Dimensions,AsyncStorage,Alert,StyleSheet,Linking, Text,TextInput, View,ScrollView,Button,TouchableOpacity,TouchableHighlight, FlatList,ImageBackground} from 'react-native';
...
componentWillMount ()
{
  ...
  this.setState({kaddi: this.props.kadi });
   ...
   firebase.database().ref('/uye/').orderByChild('kadii').once('value', (snap) => {
    ...
    firebase.database().ref('/uye_detay/'+this.props.kadimm+"/begeni/").orderByChild('kadiii').once('value', (snap) => {
      let user = [];
      let say = 0;
      snap.forEach((konum) => {
        
        const {kadiii} = konum.val();
        if(kadiii == this.props.kadi){
          user.push({kadiii});
          this.setState({begenmis : true});
  ...
  firebase.database().ref('/uye_detay/').orderByChild('kadi').once('value', (snap) => {
      ...
      snap.forEach((konum) => {
        ...
        if(kadi == this.props.kadimm){
          this.setState({gizliligim : gizlilik});
...
begeniistek = () =>{
  if(this.state.begendi !== true){
   ...
   if(this.state.gizliligim == "acik"){

        if(this.state.begenmis == true ){
        ...
        firebase.database().ref('/uye_detay/'+this.state.kaddi+"/begeniistek/"+this.state.k).set({
                  kadiii: this.state.k,
                  onay: 1,
  ...
  if(this.state.begendi == true){
     ...
     firebase.database().ref('/uye_detay/'+this.state.kaddi+"/begeniistek/"+this.state.k).remove();
